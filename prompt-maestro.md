...
│   │                                      Fuente de los TEMARIOS de reqsw/uml (solo orden/énfasis; texto jamás).
│   │                                      Queda por convertir: IA (17 PDFs en secuencia), Java/POO, Python
│   └── superpowers/
...
└── app/
    ...
    │   ├── liquid.js, liquid.css        ← window.Liquid: ripple(), reveal(), pointer(), grid() + el CSS del vidrio
...
4. **Desbloqueo**: la mecánica es **genérica por dato** (`courses.prereq_course_id` + status BLOQUEADO derivado + **403 en el SERVIDOR** para `GET /courses/:id` y `GET /lessons/:id`, no solo candado en la UI). Cadenas actuales: `bd2` exige `bd1` al 100% y `uml` exige `reqsw` al 100%. Añadir una cadena nueva = una línea de seed, cero código.
...
6. **Logros**: 17 (13 visibles + **4 secretos**). Se derivan de contadores **monótonos** (solo crecen) — por eso es seguro derivarlos: el conjunto de desbloqueados nunca encoge. La constancia se mide contra `bestStreak` (la MEJOR racha), no la actual: perder la racha no te quita el logro. **Los dos logros de catálogo** ("Sin dejar una" = las 80 lecciones; "El plan completo" = las 8 materias) **miden el catálogo vigente**: sus targets son estáticos en `achievements.js` y se actualizan a mano en cada iteración de contenido (decisión documentada — derivarlos del catálogo volvería async toda la cadena; en teoría subir el target podría re-bloquear un logro ya obtenido, pero nadie está al 100%; si algún día lo estuviera, se revisita).
...
- **El dev server suele quedar corriendo entre sesiones** (a veces gestionado por el panel del navegador vía `.claude/launch.json`, config "dev" — relanzarlo con `preview_start` es el camino correcto si está caído; NO usar Bash para servidores). Guarda el backend **en memoria**: si sus respuestas `/api` parecen viejas, **mátalo y relanza**. Los estáticos (`web/`) sí se sirven frescos del disco — para cambios de frontend basta Ctrl+Shift+R. Las queries van a la BD en vivo: tras `npm run seed`, el contenido nuevo aparece sin reiniciar.
...
**Convenciones:** commits en español (`feat:`/`fix:`/`docs:`/`chore:`/`refactor:`); TDD estricto en backend (RED→GREEN, con la evidencia en el reporte); copy en **español con tuteo**, sentence case, **sin emoji**, metadatos con "·". Colores por materia: Programación=blue, BD=cyan, Algoritmos=violet, Web=amber, Ingeniería de software=violet (compartir tono entre materias es normal).

**Dos lecciones de proceso ganadas a golpes:** (1) en los reportes de verificación de subagentes, exigir el **output literal** (getComputedStyle, conteos, payloads) — una afirmación sin evidencia no cuenta como verificación (un implementador afirmó un check de navegador que no corrió porque el server estaba caído); (2) al pedir un *rephrase* de una frase, verificar el **referente** tras el cambio ("sus nueve diagramas" — ¿de UML o del curso?): un fixer introdujo una contradicción reencuadrando con el referente equivocado.

**Contenido didáctico** (el oficio completo, destilado de las iteraciones 10-11):

- **Verdad técnica SIEMPRE**: el contenido se escribe desde el canon de la disciplina (Pressman/SWEBOK/IEEE 29148 para requisitos; UML 2.x/OMG/Fowler para modelado; y equivalentes en SQL/Java/JS). El reviewer de cada tarea revisa **como profesor**: resuelve cada quiz de forma independiente, recorre cada match par por par, y en cursos con diagramas los **lee como delineante** (reconstruye el texto visible sin spans y verifica columnas + notación canónica).
- **Copyright innegociable**: los PDFs de `docs/documentos-carrera/` aportan SOLO el orden y el énfasis del temario (capturados en el spec de cada curso) — **jamás se abren durante la implementación ni se copia una frase**. Ejemplos siempre propios, del mundo compartido biblioteca/carrito (continuidad entre cursos).
- **Reglas de ejercicios**: una única respuesta defendible; distractores plausibles pero inequívocamente incorrectos; explicaciones `ok`/`bad` que ENSEÑAN (la bad corrige el malentendido específico, no repite la ok); los índices `correct` REPARTIDOS entre lecciones sin patrón (máx ⌈n/4⌉ por índice); `match` con `right` mezclado SIN puntos fijos y 4 textos únicos (repetir significados rompe la unicidad del emparejamiento); `order` con payload en derangement; `blanks` con banco ⊇ respuestas + ≥2 distractores. Los tests de estructura por curso verifican estos invariantes.
- **Contrato de dibujo** (cursos con diagramas — vive en el spec de uml §3): caracteres de caja Unicode, ≤60 columnas y ≤12 líneas, un concepto por diagrama, color selectivo con K/S/N/C solo sobre el elemento enseñado, alineación por TEXTO VISIBLE (los spans no ocupan ancho); estados con esquinas redondeadas `╭╮╰╯` vs clases/componentes rectas `┌┐└┘` (distinción canónica deliberada); lifelines punteadas `┊` vs activaciones `┃`.
- **Mecánica seed-data**: spans de color solo dentro de template literals con las constantes K/S/N/C; jamás `${}` dentro de strings con comillas dobles. Curso nuevo = archivo en `seed-data/` + una línea en `COURSES` de `seed.js` + tests de totales/estructura — cero frontend, cero esquema (todo es data-driven). Al crecer el temario: recalibrar/mover el techo de `levels.js` (decisión con el usuario) y los 2 logros de catálogo.
