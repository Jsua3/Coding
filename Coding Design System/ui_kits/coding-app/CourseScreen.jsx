...
function CourseScreen({ data, onBack, onOpenLesson, tab, setTab }) {
  ...
  return (
    <PageFrame>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, margin: "36px 0 20px" }}>
        ...
        <span style={{ fontSize: "var(--text-sm)", color: "var(--text-tertiary)" }}>Materias / Bases de datos I</span>
      </div>
      <GlassPanel tint="cyan" padding="var(--space-7)" radius="var(--radius-xl)" style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 28 }}>
          <div style={{ flex: 1 }}>
            <Badge tone="cyan" dot>EN CURSO</Badge>
            ...
            <p style={{ margin: 0, fontSize: "var(--text-md)", color: "var(--text-secondary)", maxWidth: 560 }}>Domina tablas, claves, consultas SQL y normalizacion — la base de todo sistema de informacion.</p>
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              <Button iconLeft={<KIcon d={ICONS.play} />} onClick={() => onOpenLesson(data.units[1].lessons[1])}>Continuar leccion</Button>
              <Button variant="secondary" iconLeft={<KIcon d={ICONS.book} />}>Ver temario completo</Button>
            </div>
          </div>
          <Progress value={course.progress} shape="ring" tone="cyan" size="lg" showLabel />
        </div>
      </GlassPanel>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {data.units.map((u) => (
          <GlassPanel key={u.name} padding="var(--space-5)">
